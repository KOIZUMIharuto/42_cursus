...
/*   Updated: 2024/02/20 19:21:00 by hkoizumi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (!buf)
		return (NULL);
	can_continue_read = BUFFER_SIZE;
	line = NULL;
	while (can_continue_read == BUFFER_SIZE)
...
int	add_buf_to_line(int fd, char *buf, char **line)
{
	...
	did_read_success = BUFFER_SIZE;
	if (buf[0] == '\0')
	{
		did_read_success = read (fd, buf, BUFFER_SIZE);
